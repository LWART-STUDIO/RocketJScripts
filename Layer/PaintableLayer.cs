...
namespace DTerrain
{
    public class PaintableLayer<T> : MonoBehaviour, ILayer<T> where T:PaintableChunk
    {
        ...
        [SerializeField] private List<T> _chanks;
        public List<T> Chunks => _chanks;


        [NaughtyAttributes.Button()]
        public void Clear()
        {
            foreach (var variable in _chanks)
            {
        ...
        {
            foreach(PaintableChunk t in _chanks)
            {
                t.SortingLayerID = _sortingLayerID;
                t.Init();
            }
        }
        ...
        {
            return _chanks[GetChunkIDByPosition(position)];
        }
        ...
        private void PaintColumn(int x, int y, Range r, PaintingParameters pp)
        {
            ...
            while (true)
            {
                ...
                if (cid >= 0 && cid < _chanks.Count && k + ychunk < ChunkCountY && (k - 1) * chunkSizeY <= height)
                {
                    _chanks[cid].Paint(new RectInt(posInChunkX, posInChunkY - k * chunkSizeY, 1, r.Length+1), pp);
                    cid++;
                    k++;
                }
                else
